...
class Args:
    ...
    dataset_name: str = "SATQuest-RFT-1k"
    seed: int = 9527
...
if __name__ == "__main__":
    ...
    S_dedu = set()
    pair_cnfs = []
    for N in sorted(list(range(3, 4 + 1)) * REPEAT):
        for A in range(21, 40 + 1):
            A = A / 10
            M = int(N * A)
            print(len(pair_cnfs), N, M, A)
            while True:
    ...
    dataset_dict = DatasetDict(
        {
            "train": Dataset.from_list(cnf_item_list),
        }
    )
